...
def get_character_count(room):
  codes = ''.join(room.split('-')[0:-1])
  char_count = {}
...
def get_checksum(room):
  checksum = ''.join(room.split('-')[-1:])
  return checksum.split('[')[1].replace(']', '')

def create_checksum(char_count):
  return sorted(char_count.keys(), lambda x,y: cmp(char_count[x], char_count[y]) if char_count[x] != char_count[y] else  cmp(y, x), None, True)

def check_checksum(checksum, actual_checksum):
  index = 0
  for char in checksum:
    index = actual_checksum.find(char, index)
    if index == -1:
      return False
...
def find_room_codes():
  ...
  for room in rooms:
    char_count = get_character_count(room)
    checksum = get_checksum(room)
    actual_checksum = ''.join(create_checksum(char_count))
    print checksum
    print actual_checksum

    if check_checksum(checksum, actual_checksum):
      total += get_sectorid(room)
